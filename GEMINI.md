...
- **Shortcuts**: Checkpoints sit only on the trunk, never on a shortcut branch and never inside the stretch a branch skips. Both routes then pass every checkpoint in order and the ring needs no knowledge that a branch exists. A shortcut is kept only if carving it leaves no checkpoint that can be driven round (a flood fill, also a test), and corner run off never touches ground owned by another part of the lap: it once dug through the wall between parallel stretches on every circuit, and laps taken over those gravel bridges never counted.
- **Item Kit**: Boost, oil, banana, shield, spring, shock, puck, ghost and decoy, drawn from a flat bag. Every spin goes through `takeHit`: a ghost is never hit and a shield takes the hit instead. Rare snapshot flags (`shield`, `ghost`, `shocked`, `hop`, `wrongWay`, `falling`, `land`) are sent only while true to keep a full frame under 4 KB.
- **Jumps, Holes & Pads**: A jump holds its speed (no brake, drag or off-road cap in the air), and every ramp's landings are checked every half tile from the slowest launch to the fastest. About half the ramps have an `S_HOLE` past the lip across the whole road; a car that drives in falls for `FALL_MS` and is set down past it. A wall-jump pad (at most one per circuit, on three circuits today) flies a car a fixed distance over a wall, turns it along the stretch it lands on, and credits the checkpoints it flew past. Run off has no dead-end fingers.
- **Camera**: Eases its offset from the car, never its absolute position, so it does not trail the car at speed. `DELAY_MS` is 40.
- **Pickups & HUD**: A car takes the nearest *ready* box in `PICKUP_REACH`. Moving instanced layers (pickups, hazards, skids) are built with `dynamicLayer` and never frustum culled, because an `InstancedMesh` computes its bounding sphere once. The HUD (place, lap, leader, item, speedometer from snapshot `speed`) and the wrong-way warning (`updateWrongWay`, a rule) are drawn into the overlay canvas.
- **Ramps**: Ramp height is render only. The rules track `airUntil` and nothing else; `airT` exists for the page to draw an arc with. Giving the rules a z axis would make this a different game. The grid says where a ramp is but not which way it faces, so `carve` also returns `ramps` (`{x, y, heading, width}`) and `welcome` ships them; the page draws a wedge rising the way the lap runs. `rampRuns` reads the list from the finished grid, one entry per unbroken run of ramp tiles, because pickup pads are laid over some ramp tiles later. A car is drawn at `carLift`: on a ramp it rides the slope, and off it the jump arc sits on a line from the lip's height down to the ground, because the rules keep `airT` at 0 until the car leaves the lip.
...
- **No Live-Car Exemption**: Cutline interpolates your own car like every other (`sample(now)`, no `liveId`). Snapshots land unevenly (measured on Windows: 46 Hz, gaps of 16 or 30 ms), and the newest-snapshot exemption froze the car on 67 of 165 frames, which read as stuttering behind a chase camera. `heading` is blended too, the short way round, or the car's facing steps through corners at the same uneven rhythm.
